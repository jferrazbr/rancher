...
func (f *fakeETCDSnapshotSaveController) EnqueueAfter(namespace, name string, duration time.Duration) {
	f.enqueueCalls = append(f.enqueueCalls, enqueueCall{namespace, name, duration})
}

func (f *fakeETCDSnapshotSaveController) Delete(namespace, name string, options *metav1.DeleteOptions) error {
...
func TestOnChange_StablePausedOperation(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, 0, len(controller.deleteCalls), "Delete should not be called")

			// Verify no enqueue occurred
			assert.Equal(t, 0, len(controller.enqueueCalls), "EnqueueAfter should not be called")
		})
	}
}
